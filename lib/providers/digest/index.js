var DigestAutograph = function(baseUrl, definition,options) {
	var self = this;
	this.handle401 = function(response,request, callback) { 
	console.log("Handling the 401");
		self.setAuthChallenge(response.headers['www-authenticate']); 
		callback();
...
return challenge;
	...
	this.setAuthChallenge = function(challenge) {
		var self = this;
		self.challenge = _parseChallenge(challenge);
	}	
	this.nc = 0;
...
console.log("Heasder",header);
	...
	this.signRequest = function(request) { 
		...
		return { request: request, handler401: this.handle401 };
	}
